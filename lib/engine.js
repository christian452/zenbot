...
module.exports = function container (get, set, clear) {
  ...
  return function (s) {
        ...
        getQuote(function (err, quote) {
          ...
          if (signal === 'buy') {
            ...
            if (canTransact(s.product, size, price)) {
              ...
              console.error('Could not place sell order')
    ...
    function canTransact (product, size, price) {
      if (!product.min_size) {
        msg('Min size not set in product.')
      }
      if (!product.min_total) {
        msg('Min total not set in product.')
      }

      if (Number(size) < Number(product.min_size)) {
      ...
      if (total < Number(product.min_total)) {
