        ...
        s.exchange[type](orderCopy, function (err, apiOrder) {
              ...
              function checkHold () {
                s.exchange.getOrder({order_id: order.order_id, product_id: s.product_id}, function (err, apiOrder) {
                  ...
                  syncBalance(function (err) {
                    if (err) return cb(err)
                    msg(`Checking if we have balance on hold ${JSON.stringify(s.balance)}`)
                    if (!(s.balance.asset_hold > 0 || s.balance.currency_hold > 0)) {
                      cb(null, doReorder ? null : false)
                    }
                    ...
                    let onHold
                    if (type === 'buy') {
                      onHold = n(s.balance.currency).subtract(s.balance.currency_hold || 0).value() < n(order.price).multiply(order.remaining_size).value()
                    }
                    else {
                      onHold = n(s.balance.asset).subtract(s.balance.asset_hold || 0).value() < n(order.remaining_size).value()
                    }
                    if (onHold) {
                      // wait a bit for settlement
                      msg('funds on hold after cancel, waiting 5s')
                      return setTimeout(checkHold, c.wait_for_settlement)
                    } else {
                      cb(null, doReorder ? null : false)
                    }
                  })
          ...
          function checkOrder () {
            ...
            s.exchange.getOrder({order_id: order.order_id, product_id: s.product_id}, function (err, apiOrder) {
              ...
              if (order.status === 'rejected' && (order.reject_reason === 'post only' || apiOrder.reject_reason === 'post only')) {
                msg('post-only ' + type + ' failed, re-ordering')
                return cancelOrder(true)
              }

              if (order.status === 'rejected') {
                msg(`Order rejected, reordering...`)
                return cancelOrder(true)
              }

              if (new Date().getTime() - order.local_time >= so.order_adjust_time) {
                msg(`Adjusting order ${JSON.stringify(order)}`)
                getQuote(function (err, quote) {
                  ...
                  if (type === 'buy') {
                    ...
                    if (n(order.price).value() < markedPrice) {
                      ...
                      order.local_time = new Date().getTime()
                      return setTimeout(checkOrder, so.order_poll_time)
                    }
                  } else {
    ...
    function executeSignal (signal, _cb, size, isReorder, isTaker) {
      ...
      const cb = function (err, order) {
        ...
        if (err) {
          if (_cb) {
            _cb(err)
          } else if (err.message && err.message.match(niceErrors)) {
            msg((err.message + ': ' + err.desc).red)
          } else {
            //memDump()
            msg('\n')
            msg(err)
      ...
      syncBalance(function (err) {
        ...
        getQuote(function (err, quote) {
          ...
          if (signal === 'buy') {
            ...
            msg(`Buying price ${price}`)
            msg(`Balance ${JSON.stringify(s.balance)}`)
            msg(`Quote ${JSON.stringify(quote)}`)
            if (!size) {
            ...
            if (canTransact(s.product, size, price)) {
              ...
              if (so.max_sell_loss_pct && sellLoss > so.max_sell_loss_pct) {
                ...
                if (
                  (s.balance.asset_hold != 0) &&
                  n(s.balance.asset).subtract(s.balance.asset_hold || 0).value() < n(size).value()
                ) {
        ...
        placeOrder(signal, {
          ...
          if (!order) {
            if (order === false) {
              ...
              return cb('not enough balance, or signal switched, cancel ' + signal)
            }
            if (s.last_signal !== signal) {
            ...
            return executeSignal(signal, cb, remainingSize, true)
          }
          cb(null, order)
        })
      }
